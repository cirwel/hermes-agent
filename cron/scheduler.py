...
def run_job(
    ...
    try:
        ...
        _audit.write(dict(result, response_silent=_audit_response_silent), None)
...
def run_one_job(
    ...
    # Bind this attempt's immutable identity for the whole in-process run, so
    # every tool and child process launched underneath reports the job and
    # execution it actually belongs to. ContextVars, not process env: parallel
    # jobs share the interpreter and would otherwise clobber each other.
...
def _save_compose_deliver(
    ...
    # A silence marker is a control token, not report punctuation. If the agent
    # mixes it with substantive content, the full output is already saved above;
    # fail the run before the irreversible send so only the compact failure
    # summary can reach the operator.
    if d.success and _is_cron_silence_contract_violation(final_response):
        d.success = False
        d.error = (
    ...
    if d.should_deliver and d.success and _is_cron_silence_response(deliver_content):
        ...
        # Mixed marker/content responses became failed runs above, so only exact
        # markers can reach this successful-suppression branch.
